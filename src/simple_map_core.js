...
function isInherited(r){return !!(r&& (r.inherited||r.dataLevel==='city-inherited'||String(r.quality||'').includes('用于区县底色')))} function levelText(r){if(!r?.price)return '';if(isInherited(r))return '沿用市均';if(r.dataLevel==='district'||String(r.quality||'').includes('区县'))return '区县数据';if(r.dataLevel==='city')return '城市数据';return r.supplemental?'补充口径':'公开数据'} function levelClass(r){return isInherited(r)?'levelBadge inherited':(r.dataLevel==='district'||String(r.quality||'').includes('区县')?'levelBadge district':'levelBadge')} function labelText(r){return escapeHtml(r.label)+' '+fmtWan(r.price)+(isInherited(r)?' 市均':'')} function sourceLine(r){return [levelText(r),r.quality,r.source].filter(Boolean).join(' · ')}
